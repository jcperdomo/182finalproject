...
class Game(object):
    ...
    def playGame(self):
        ...
        """
        deck = cards.allCards()
        hands = cards.dealHands(deck, 52/8)
        agents = [agentConstructor(hand)
                  for agentConstructor, hand in zip(self.agents, hands)]
        
        # randomly choose a player with a 3 to start
        # (prop. to how many 3's they have)
        ...
        while not curState.isFinalState():
            ...
            # make the move - take out cards from hand, update the state
            self.hand[whichCard] -= numCards
        ...
        # the game is over, return the results
        return results
